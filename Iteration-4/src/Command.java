...
class favoriteCommodity implements Command {
    private boolean validateDirectedCommodityExistence(String shopId, String commodityId) {
        Shop shop = Global.getGlobalShops().get(shopId);
        if (shop.getCommodityOfShop().containsKey(commodityId)) {
            return shop.getCommodityOfShop().get(commodityId).getCommodityUnit().getAvailability();
    ...
    public void execute(String[] args) {
        ...
        if (!validateCount(commodityCount)) {
            System.out.println("Illegal commodity quantity");
            return;
        }
        Shop shop = Global.getGlobalShops().get(shopId);
        Commodity commodity = shop.getCommodityOfShop().get(commodityId);
        int count = Integer.parseInt(commodityCount);
        for (Favorite favorite: Global.getCurrentLoginUser().getMyFavoriteCommodity().values()) {
            if (favorite.getAffiliCommodityId().equals(commodityId) && favorite.getAffilShopId().equals(shopId)) {
                favorite.changeCount(count);
                System.out.println("Favorite commodity success");
                return;
            }
        }
        Global.getCurrentLoginUser().favoriteCommodity(commodityId, commodity, count, shopId);
        System.out.println("Favorite commodity success");
    }
}

class cancelFavoriteCommodity implements Command {
    private boolean validateFavoriteCommodityExistence(String commodityId, String shopId) {
        for (Favorite favorite: Global.getCurrentLoginUser().getMyFavoriteCommodity().values()) {
            if (favorite.getAffiliCommodityId().equals(commodityId) && favorite.getAffilShopId().equals(shopId)) {
                return true;
            }
    ...
    public void execute(String[] args) {
        ...
        if (!validateFavoriteCommodityExistence(commodityId, shopId)) {
            System.out.println("Favorite not exists");
            return;
        }
        Global.getCurrentLoginUser().cancelFavoriteCommodity(commodityId, shopId);
        System.out.println("Cancel favorite commodity success");
    }
}

class listFavoriteCommodity implements Command {
    private boolean validateFavoriteCommodityExistence() {
        if (Global.getCurrentLoginUser().getMyFavoriteCommodity().isEmpty()) {
            return false;
        }
        return true;
    }
    private void printFavoriteCommodityInfo() {
        HashMap<String, Favorite> favorites= Global.getCurrentLoginUser().getMyFavoriteCommodity();
        List<Map.Entry<String, Favorite>> keys = new ArrayList<>(favorites.entrySet());
        keys.sort(new Comparator<Map.Entry<String, Favorite>>() {
            @Override
            public int compare(Map.Entry<String, Favorite> o1,
                               Map.Entry<String, Favorite> o2) {
                // 顺序排序，如果想要逆序，o2 - o1 即可
                return o1.getValue().getSerial() -
                        o2.getValue().getSerial();
            }
        });
        for (Map.Entry<String, Favorite> entry : keys) {
            String key = entry.getKey();
            Favorite currentFavorite = favorites.get(key);
            String formattedPrice = String.format("%.2f", Global.getCommodityUnits().get(currentFavorite.getAffiliCommodityId()).getPrice());
            System.out.println(currentFavorite.getAffilShopId() + ": " + currentFavorite.getAffiliCommodityId() +
                    " " + currentFavorite.getCount() +
                    " " + formattedPrice + "yuan");
        }
    }
    @Override
    public void execute(String[] args) {
        ...
        if (!validateFavoriteCommodityExistence()) {
            System.out.println("Favorite not exists");
            return;
        }
        printFavoriteCommodityInfo();
    }
}

class uploadFavoriteCommodity implements Command {
    private boolean validateFavoriteCommodityExistence() {
        if (Global.getCurrentLoginUser().getMyFavoriteCommodity().isEmpty()) {
            return false;
        }
    ...
    public void execute(String[] args) {
        ...
        String filePath = args[0];
        filePath = filePath.contains("./") ? "./data/" + filePath.substring(1) : "./data/" + filePath;
        FileOperation.uploadFavorites(filePath, Global.getCurrentLoginUser().getMyFavoriteCommodity());
        System.out.println("Upload favorite commodity success");
    }
}
class readFavoriteCommodity implements Command {
    ...
    public void execute(String[] args) {
        ...
        FileOperation.readFavorites(filePath, Global.getCurrentLoginUser());
        System.out.println("Read favorite commodity success");
    }
}
class buyFavoriteCommodity implements Command {
    private boolean validateDirectedCommodityExistence(String shopId, String commodityId) {
        Shop shop = Global.getGlobalShops().get(shopId);
        if (shop.getCommodityOfShop().containsKey(commodityId)) {
            return shop.getCommodityOfShop().get(commodityId).getCommodityUnit().getAvailability();
    ...
    private void buyCommodity(String shopId, String commodityId, String countString) {
        ...
        Commodity commodity = shop.getCommodityOfShop().get(commodityId);
        Global.getCurrentLoginUser().createMyOrder(shop, commodity, count);
        String currentOrderId = "O-" + Global.getCurrentOrderId();
        Order currentOrder = Global.getCurrentLoginUser().getMyOrder().get(currentOrderId);
        String formattedCost = String.format("%.2f", currentOrder.getTotalCost());
        System.out.println(currentOrder.getId() + ": " + currentOrder.getAffilShop().getId() +
                " " + currentOrder.getAffilCommodity().getCommodityUnit().getId() +
                " " + currentOrder.getBuyCount() +
    ...
    public void execute(String[] args) {
        ...
        for (Map.Entry<String, Favorite> entry : keys) {
            String key = entry.getKey();
            Favorite favorite = Global.getCurrentLoginUser().getMyFavoriteCommodity().get(key);
            buyCommodity(favorite.getAffilShopId(), favorite.getAffiliCommodityId(), Integer.toString(favorite.getCount()));
            Global.getCurrentLoginUser().getMyFavoriteCommodity().remove(key);
        }
    }
}